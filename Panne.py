...
import logging
import ast
from PanneExtract import Classification
import re 
import serial
...
# Publish num_messages mesages to the MQTT bridge once per second.
    ...
    while(True):
      ...
      # data = ser.readlines() 
        ...
        data = text.readlines()
        for j in data:
            code = re.findall(r'(?<=code:)[0-9-a-zA-Z]+', j)
            voiture = re.findall(r'(?<=uic:)[0-9]+', j)
            date = datetime.datetime.utcnow()
            date_str=time.strftime("%m/%d/%YT%H:%M:%S")
            if code !=[] :
                equi = Classification(code)
                jasonData= {
                    "Code" : code[0] ,
                    "Voiture" : voiture[0],
                    "equi"  : equi ,
